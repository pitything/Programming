package com.atguigu._2fruit_mvc_reflect.servlet;
...
public class FruitServlet extends ViewBaseServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        /** 使用反射进行匹配 */
        try {
            Class<?> fruitServlet = FruitServlet.class;
            Method method = fruitServlet.getDeclaredMethod(operate, HttpServletRequest.class, HttpServletResponse.class);
            if(null != method) {
                method.invoke(this, request, response);
                // 不能如下写，会有空指针，因为FruitServlet是tomcat管理的，一些配置如servletConfig、servletContext会自动创建。
                // method.invoke(new FruitServlet(), request, response);
            }else{
    ...
    private void edit(HttpServletRequest request , HttpServletResponse response)throws IOException, ServletException {
        ...
        if(StringUtil.isNotEmpty(fidStr)){
            ...
            super.processTemplate("edit_mvc",request,response);
        }
    }
    ...
    private void index(HttpServletRequest request , HttpServletResponse response)throws IOException, ServletException {
        ...
        super.processTemplate("index_mvc",request,response);
    }
}
