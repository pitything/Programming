package Algorithm.LeetCode.leetcode.editor.cn;//给你两个单链表的头节点 headA 和 headB ，请你找出并返回两个单链表相交的起始节点。如果两个链表不存在相交节点，返回 null 。
//
// 图示两个链表在节点 c1 开始相交： 
//
// 
...
class Solution160 {
    ...
    /**
     * 数学方法
     * 思路：用pa、pb表示headA和headB，分别遍历pa和pb到结尾，然后pa的指针指向headB，pb的指针指向headA，直到pa == pb
     *      说明：利用特性： headA.len + headB到交点的距离 = headB.len + headA到交点的距离
     * 复杂度：时间：O(m+n) 空间：O(1)
     * @param headA
     * @param headB
     * @return
    ...
    /**
     * 思路：求得headA和headB的长度，如果lenA > lenB ，则headA往后推，直到长度相等。
     *      从相等长度开始遍历，如果存在一个相同节点，则返回，否则返回null。
     * 复杂度：时间：O(m+n) 空间：O(1)
     * @param headA
     * @param headB
     * @return
