...
> - **添加元素的过程**
>
>   1. 向HashMap中添加entry1(key，value)，需要首先计算entry1中key的哈希值(根据key所在类的hashCode()计算得到)，此哈希值经过某种算法，得到在底层Entry[]数组中要存储的位置i。
>    
...
>   4. 如果hash值相同，继续比较二者的equals方法。
>   5. 如果equals返回值为true，则使用entry1的value去替换equals为true的entry的value。
>   6. 如果遍历一遍以后，发现所有的equals返回都为false,则entry1仍可添加成功。entry1指向原有的entry元素。
>   
>- **HashMap的扩容**
> 
...
>
> - HashMap的hash()方法**(h = key.hashCode()) ^ (h >>> 16)**获取到节点的hash，取下标索引
>
